...
QUADRATIC = 2
cubic = 3
numberOfSquareElements = 7
numberOfArmElements = 7
numberOfLengthElements = 3

pipeRadius = 0.5  # 12
lengthSize = 3.000  # 24
squareSizeRatio = 0.5000
...
gamma = 1.00 / (1.00 + 2.500 * (vol_frac))
beta = viscosity / gamma

d = .500
perm = d ** 2.000 * (1.000 - vol_frac) ** 3.000 / (180.00 * vol_frac ** 2.000)
perm = 0.0052
perm_visc = perm / viscosity
#beta = beta*(perm_visc)*(1/porosity)
print('perm=',perm)
print('perm_visc ratio=',perm_visc)
print(porosity, perm)
uInterpolation = QUADRATIC
pInterpolation = LINEAR
v_z = (perm_visc) * (Pdrop / lengthSize)
print('minput velocity =',v_z)
...
numberOfFluidNodesPerBlock = numberOfSquareElements * (numberOfNodesXi - 1) * (
            ...
            for xElementIdx in range(1, numberOfSquareElements + 1):
                ...
                if (xElementIdx == 1):
                    localNodes[localNodeIdx000] = (
                                                              previousBlock - 1) * numberOfFluidNodesPerBlock + numberOfSquareElements * (
                                                              numberOfNodesXi - 1) + \
                                                  (yElementIdx - 1) * (numberOfNodesXi - 1) * numberOfSquareElements * (
                                                              numberOfNodesXi - 1) + \
                                                  (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                                                              numberOfNodesXi - 1)
                    ...
                    localNodes[localNodeIdx000] = (blockIdx - 1) * numberOfFluidNodesPerBlock + (xElementIdx - 2) * (
                ...
                localNodes[localNodeIdx110] = localNodes[localNodeIdx100] + numberOfSquareElements * (
                            numberOfNodesXi - 1) * (numberOfNodesXi - 1)
                localNodes[localNodeIdx001] = localNodes[localNodeIdx000] + numberOfFluidNodesPerLength * (
                            numberOfNodesXi - 1)
                localNodes[localNodeIdx101] = localNodes[localNodeIdx100] + numberOfFluidNodesPerLength * (
                            numberOfNodesXi - 1)
                localNodes[localNodeIdx011] = localNodes[localNodeIdx010] + numberOfFluidNodesPerLength * (
                            numberOfNodesXi - 1)
                localNodes[localNodeIdx111] = localNodes[localNodeIdx110] + numberOfFluidNodesPerLength * (
                            numberOfNodesXi - 1)
    ...
    if (numberOfSquareElements == 1):
        ...
        if (debug):
            ...
            if (uInterpolation == QUADRATIC):
                ...
                print('                             %8d, %8d, %8d, %8d, %8d, %8d, %8d, %8d, %8d' % \
                      (localNodes[18], localNodes[19], localNodes[20], localNodes[21], localNodes[22], localNodes[23],
                       localNodes[24], localNodes[25], localNodes[26]))
        ...
        for yElementIdx in range(1, numberOfSquareElements + 1):
            for xElementIdx in range(1, numberOfSquareElements + 1):
                localNodes = [0] * numberOfLocalNodes
                elementNumber = 4 * numberOfFluidElementsPerBlock + xElementIdx + (
                ...
                if (yElementIdx == 1):
                    if (xElementIdx == 1):
                        ...
                        localNodes[localNodeIdx000] = 3 * numberOfFluidNodesPerBlock + \
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                                                                  numberOfNodesXi - 1)
                        localNodes[localNodeIdx100] = 3 * numberOfFluidNodesPerBlock + numberOfArmElements * (
                                    numberOfNodesXi - 1) * \
                                                      numberOfSquareElements * (numberOfNodesXi - 1) + (
                                                                  numberOfNodesXi - 1) + \
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                        ...
                        localNodes[localNodeIdx010] = 3 * numberOfFluidNodesPerBlock - (numberOfNodesXi - 1) + \
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                                                                  numberOfNodesXi - 1)
                        localNodes[localNodeIdx110] = 4 * numberOfFluidNodesPerBlock + (
                                    numberOfSquareElements * (numberOfNodesXi - 1) - 1) * \
                        ...
                        localNodes[localNodeIdx100] = 4 * numberOfFluidNodesPerBlock + \
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                                                                  numberOfNodesXi - 1)
                        localNodes[localNodeIdx010] = 4 * numberOfFluidNodesPerBlock + (
                                    numberOfSquareElements * (numberOfNodesXi - 1) - 1) * \
                        ...
                        # Bottom
                        localNodes[localNodeIdx000] = 3 * numberOfFluidNodesPerBlock + numberOfSquareElements * (
                                    numberOfNodesXi - 1) * \
                                                      numberOfArmElements * (numberOfNodesXi - 1) + (
                    ...
                    if (xElementIdx == 1):
                        # Top-left
                        localNodes[localNodeIdx000] = 2 * numberOfFluidNodesPerBlock + numberOfSquareElements * (
                                    numberOfNodesXi - 1) * \
                                                      numberOfArmElements * (numberOfNodesXi - 1) + (
                                                                  numberOfNodesXi - 1) + \
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                        ...
                        # Top-right
                        localNodes[localNodeIdx000] = 4 * numberOfFluidNodesPerBlock + (
                                    numberOfSquareElements * (numberOfNodesXi - 1) - 1) * \
                                                      ((numberOfSquareElements - 1) * (numberOfNodesXi - 1) - 1) + \
                        ...
                        localNodes[localNodeIdx100] = numberOfSquareElements * (numberOfNodesXi - 1) * \
                                                      ...
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                                                                  numberOfNodesXi - 1)
                        localNodes[localNodeIdx010] = numberOfFluidNodesPerBlock + numberOfSquareElements * (
                                    numberOfNodesXi - 1) * \
                                                      numberOfArmElements * (numberOfNodesXi - 1) + (
                                                                  numberOfNodesXi - 1) + \
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                        ...
                        localNodes[localNodeIdx000] = 4 * numberOfFluidNodesPerBlock + (
                                    numberOfSquareElements * (numberOfNodesXi - 1) - 1) * \
                                                      ...
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                                                                  numberOfNodesXi - 1)
                        localNodes[localNodeIdx100] = localNodes[localNodeIdx000] + (numberOfNodesXi - 1)
                        localNodes[localNodeIdx010] = 2 * numberOfFluidNodesPerBlock - (xElementIdx - 1) * (
                                    numberOfNodesXi - 1) + \
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                                                                  numberOfNodesXi - 1)
                        localNodes[localNodeIdx110] = localNodes[localNodeIdx010] - (numberOfNodesXi - 1)
                    ...
                    if (xElementIdx == 1):
                        # Left
                        localNodes[localNodeIdx000] = 3 * numberOfFluidNodesPerBlock - (yElementIdx - 1) * (
                                    numberOfNodesXi - 1) + \
                                                      (zElementIdx - 1) * numberOfFluidNodesPerLength * (
                                                                  numberOfNodesXi - 1)
                        localNodes[localNodeIdx100] = 4 * numberOfFluidNodesPerBlock + (
...
for zNodeIdx in range(1, numberOfLengthElements * (numberOfNodesXi - 1) + 2):  # 1 to 4
    ...
    for blockIdx in range(1, 5):
        for yNodeIdx in range(1, numberOfArmElements * (numberOfNodesXi - 1) + 2):  # 1 to 4
            for xNodeIdx in range(1, numberOfSquareElements * (numberOfNodesXi - 1) + 1):  # 1 to 3
                nodeNumber = (blockIdx - 1) * numberOfFluidNodesPerBlock + xNodeIdx + (
                ...
                if (nodeDomain == computationalNodeNumber):
                    ...
                    if (yNodeIdx == numberOfArmElements * (numberOfNodesXi - 1) + 1):  # if it is the last loop
                        # On the square
                        if (blockIdx == 1):
                            xPosition = squareSize - xNodeIdx * squareSize / (
                                        numberOfSquareElements * (numberOfNodesXi - 1))
                            ...
                            xPosition = xNodeIdx * squareSize / (
                                        numberOfSquareElements * (numberOfNodesXi - 1)) - squareSize
                            yPosition = -xNodeIdx * squareSize / (numberOfSquareElements * (numberOfNodesXi - 1))
                        ...
                        theta = xNodeIdx * math.pi / (2 * numberOfSquareElements * (numberOfNodesXi - 1)) + (
                                    blockIdx - 1) * math.pi / 2.0
    ...
    for yNodeIdx in range(2, numberOfSquareElements * (numberOfNodesXi - 1) + 1):
        for xNodeIdx in range(2, numberOfSquareElements * (numberOfNodesXi - 1) + 1):
            nodeNumber = 4 * numberOfFluidNodesPerBlock + (xNodeIdx - 1) + (yNodeIdx - 2) * (
                        numberOfSquareElements * (numberOfNodesXi - 1) - 1) + \
                         (zNodeIdx - 1) * numberOfFluidNodesPerLength
            nodeDomain = fluidDecomposition.NodeDomainGet(nodeNumber, 1)
            if (nodeDomain == computationalNodeNumber):
                xPosition = squareSize * ((xNodeIdx - 1) - (yNodeIdx - 1)) / (
                            numberOfSquareElements * (numberOfNodesXi - 1))
                yPosition = squareSize * ((xNodeIdx - 1) + (yNodeIdx - 1)) / (
                            numberOfSquareElements * (numberOfNodesXi - 1)) - squareSize
                zPosition = float(zNodeIdx - 1) * lengthSize / float(numberOfLengthElements * (numberOfNodesXi - 1))
                if (zNodeIdx == 1):
                    top_nodes.append(nodeNumber)
...
print(side_nodes_top)

for i in range(0, len(top_nodes)):
    z_val = fluidGeometricField.ParameterSetGetNode(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,1,iron.GlobalDerivativeConstants.NO_GLOBAL_DERIV,int(top_nodes[i]),3)
    x_val = fluidGeometricField.ParameterSetGetNode(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,1,iron.GlobalDerivativeConstants.NO_GLOBAL_DERIV,int(top_nodes[i]),1)
    y_val = fluidGeometricField.ParameterSetGetNode(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,1,iron.GlobalDerivativeConstants.NO_GLOBAL_DERIV,int(top_nodes[i]),2)
    if (pressure_bc):
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(top_nodes[i]), 4,
                                   iron.BoundaryConditionsTypes.FIXED, Pdrop)
    else:
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(top_nodes[i]), 3,iron.BoundaryConditionsTypes.FIXED, v_z)#*(1-(((x_val**2)+(y_val**2))/(100*100))))
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(top_nodes[i]), 2,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vy at top node  = 0
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(top_nodes[i]), 1,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vx at top node  = 0
        #boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(bottom_nodes[i]), 4,
                                   #iron.BoundaryConditionsTypes.FIXED, Pdrop)

for i in range(0, len(side_nodes_top)):
    if (pressure_bc):
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_top[i]), 2,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_top[i]), 1,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_top[i]), 4,
                                   iron.BoundaryConditionsTypes.FIXED, Pdrop)
    else:
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_top[i]), 3,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_top[i]), 2,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vy at side top node  = 0
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_top[i]), 1,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vx at side top node  = 0
        #boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(bottom_nodes[i]), 4,
        #                           iron.BoundaryConditionsTypes.FIXED, Pdrop)
...
for i in range(0, len(bottom_nodes)):
    if (pressure_bc):
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(bottom_nodes[i]), 4,
                                   iron.BoundaryConditionsTypes.FIXED, Pbase)
    else:
        #boundaryConditions.SetNode(dependentField,iron.FieldVariableTypes.U,1,1,int(bottom_nodes[i]),3,iron.BoundaryConditionsTypes.FIXED,v_z*(1-(x_val**2+y_val**2)/100))
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(bottom_nodes[i]), 2,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vy at top node  = 0
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(bottom_nodes[i]), 1,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vx at top node  = 0
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(bottom_nodes[i]), 4,
                                   iron.BoundaryConditionsTypes.FIXED, Pbase)
...
for i in range(0, len(side_nodes_bottom)):
    if (pressure_bc):
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_bottom[i]), 2,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_bottom[i]), 1,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_bottom[i]), 4,
                                   iron.BoundaryConditionsTypes.FIXED, Pbase)
    else:
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_bottom[i]), 3,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_bottom[i]), 2,
                                   iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vy at top node  = 0
        boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes_bottom[i]), 1,
...
for i in range(0, len(side_nodes)):
    boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes[i]), 3,
                               iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vz at top node  = 0
    boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes[i]), 2,
                               iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vy at top node  = 0
    boundaryConditions.SetNode(dependentField, iron.FieldVariableTypes.U, 1, 1, int(side_nodes[i]), 1,
                               iron.BoundaryConditionsTypes.FIXED, 0.0)  # Value of Vx at top node  = 0
...
print(v_z, ' ', perm, ' ', numberOfFluidElements)
# -----------------------------------------------------------------------------------------------------------
# OUTPUT
# -----------------------------------------------------------------------------------------------------------
...
for i in range(0, Total_nodes):
    x[i] = fluidGeometricField.ParameterSetGetNodeDP(iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES, 1,
                                                     1, int(i + 1), 1)
    y[i] = fluidGeometricField.ParameterSetGetNodeDP(iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES, 1,
                                                     1, int(i + 1), 2)
    z[i] = fluidGeometricField.ParameterSetGetNodeDP(iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES, 1,
                                                     1, int(i + 1), 3)
    p[i] = dependentField.ParameterSetGetNodeDP(iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES, 1, 1,
                                                int(i + 1), 4)
    vz[i] = dependentField.ParameterSetGetNodeDP(iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES, 1, 1,
                                                 int(i + 1), 3)
    vy[i] = dependentField.ParameterSetGetNodeDP(iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES, 1, 1,
                                                 int(i + 1), 2)
    vx[i] = dependentField.ParameterSetGetNodeDP(iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES, 1, 1,
                                                 int(i + 1), 1)
...
g = open("D:/Projects/repro-examples/uteroplacental/darcy-models/00-darcy-testcase/output/StaticDarcy.part0.exnode")
currentline = 1
delP = (Pdrop - Pbase) / lengthSize
gamma = 1 / (1 + (2.5 * (1 - porosity)))  # This term is in the equation...
...
sum1 = 0.0
avg = 0.0
with open("D:/Projects/repro-examples/uteroplacental/darcy-models/00-darcy-testcase/output/StaticDarcy.part0.exnode") as file:
...
for n in range(1, count):
    x_val = round(float(ST[21 + (n - 1) * 15].strip()), 2)  # Find the x value at the node...
    y_val = round(float(ST[22 + (n - 1) * 15].strip()), 2)  # Find the y-value at the node...
    z_val = round(float(ST[23 + (n - 1) * 15].strip()), 2)  # Find the z-value at the node...
    if (z_val == z_lim):  # Penultimate value...
        f2 = round(float(ST[26 + (n - 1) * 15].strip()), 2) * 1e-2  # Find the functional value at the node...
        r1 = np.sqrt((x_val ** 2) + (y_val ** 2))
        ratio = (iv(0, (C1 * r1 * 1e-2))) / (iv(0, (C1 * 5 * 1e-2)))
        f1 = perm_vis * delP * (1 - ratio)
        r.append(r1)
        f.append(f1)
        fd.append(f2)
    if (z_val == z_lim * 0.5):  # Midpoint value ...
        f2 = round(float(ST[26 + (n - 1) * 15].strip()), 2) * 1e-2  # Find the functional value at the node...
        r1 = np.sqrt((x_val ** 2) + (y_val ** 2))
        ratio = (iv(0, (C1 * r1 * 1e-2))) / (iv(0, (C1 * 5 * 1e-2)))
        f1 = perm_vis * delP * (1 - ratio)
